...
void chamaPerceptron(){
  ...
  if(millisAtual >= tempoPerceptron && !alteracaoUsuario){
    ...
    tempoPerceptron = 4000;//sempre que executado o perceptron volta a executar a cada 2 segundos
  ...
  if(alteracaoUsuario){//caso o usuario altere antes do perceptron eh realizado verificacao e correcao dos valores
    ...
    bool lampadaLigada = digitalRead(statusLampada);
    //após interração pelo usuário o perceptron fica 5 minutos sem executar
    if(lampadaLigada && tempoPerceptron <= 5000)
      tempoPerceptron = 300000 - (tempoPerceptron*tempoDesliga);
    else if(tempoPerceptron <= 5000)
      tempoPerceptron = 300000;
    luminosidade = getLuminosidade();
    movimento = getMovimento();
