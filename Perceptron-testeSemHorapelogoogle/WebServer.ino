...
bool saveAlteration = false;
String alterationAction = " ";
void iniciaServidor(){
  server.begin();
}
...
void conexaoServidor(){
  ...
  if (!client) {
    if(saveAlteration){
      saveAlteration = false;
      alteracaoUsuario = true;
      SaveValues(alterationAction);
    }
    novaHora();
    chamaPerceptron();
    return;
  ...
  if (req.indexOf("rele_on") != -1){    
    if(ligaLampada()){
      saveAlteration = true;
      alterationAction = "Usuario ligou pela Web   ";      
    }    
  } else if (req.indexOf("rele_off") != -1) {    
    if(desligaLampada()){
      saveAlteration = true;
      alterationAction = "Usuario desligou pela Web";      
    }
  }
  else {
  ...
  buf += tempoDesliga*(tempoPerceptron/1000);
